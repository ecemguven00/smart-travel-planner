...
def main():
    ...
    #correlation matrix grafiği pcadan önce yapıyoruz

    print("\n" + "=" * 60)
    print("1.5 Feature Correlation Matrix")
    print("=" * 60)
    ...
    plot_cumulative_variance(
    explained_variance,
    save_path=os.path.join(plots_dir, "pca_cumulative_variance.png")
)
    ...
    clustered_df_pca, kmeans_model_pca, scaler_pca, pca_model_pca, silhouette_avg_pca = apply_kmeans_clustering(
        df,
        n_clusters=6,  # Sabit küme sayısı
        use_pca=True,
        pca_components=11
    )
